...
import handlerProvider, { getPercent, regionalOrderInfo } from "../../utils/functions";
...
const handler = async(ctx: BotCtx) =>{
    ...
    const orderValue = await REGIONALORDERS.aggregate([
    ...
    if(order && order.status === "pending"){
        ...
        await REGIONALORDERS.findByIdAndUpdate(data,{driverId: ctx.from.id, status:"accepted"});
...
const acceptRegionalOrder = ctx => handlerProvider(ctx, handler);
export default acceptRegionalOrder